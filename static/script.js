...
function loadNotebook(notebook) {
    ...
    fetch(`/load/${notebook}`, { signal: loadAbortController.signal })
...
function autoSave() {
    ...
    fetch('/save', {
        ...
        body: JSON.stringify({ content, notebook: savingNotebook }),
    ...
    .catch(error => {
        if (error.name === 'AbortError') return;
        if (savingNotebook !== currentNotebook) return;
